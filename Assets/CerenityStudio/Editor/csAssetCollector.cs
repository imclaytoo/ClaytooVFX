...
namespace CerenityStudio
{
    public class csAssetCollector
    {
        ...
        public void Collection(string[] collectionFolders)
        {
            try
            {
                var serialize = new XmlSerializer(typeof(List<csCollectionData>));
                ...
                if (File.Exists(EXPORT_XMP_PATH))
                {
                    ...
                    {
                        referenceCollection = (List<csCollectionData>) serialize.Deserialize(fileStream);
                        fileStream.Close();
                    }
                }
                ...
                if (useCodeStrip)
                {
                    collectionList.Add(new csClassReferenceCollection(saveEditorExtensions));
                }

                collectionList.AddRange(new IReferenceCollection[]
                {
                    new csShaderReferenceCollection(),
                    new csAssetReferenceCollection()
                });
        ...
        private void UnregisterReferenceFromIgnoreList()
        {
            ...
            {
                var code = csClassReferenceCollection.StripComment(File.ReadAllText(path));
